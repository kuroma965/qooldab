...
export default function ProductCard({ product = {}, index = 0, itemsVisible = true }) {
  ...
  return (
    <div
      className={`bg-gradient-to-b from-gray-900 to-gray-950 rounded-2xl shadow-lg overflow-hidden border border-purple-800 transform transition-all duration-300 ${outOfStock ? '' : 'hover:scale-104 hover:shadow-xl'} ${enterClass} ${disabledCardClasses}`}
    ...
    >
      <div className="relative h-48 w-full bg-gray-800">
        ...
        {/* BADGES */}
        {isNew && !outOfStock && (
          <div className="absolute top-3 left-3 bg-green-600 text-white text-xs font-bold px-2 py-1 rounded-md">ใหม่</div>
        )}

        {outOfStock && (
          <div className="absolute top-3 left-3 bg-gray-700 text-white text-xs font-bold px-2 py-1 rounded-md">สินค้าหมด</div>
        )}
      ...
      <div className="p-3">
        ...
        <div className="flex items-center justify-between mt-3 mb-4">
          <div className="bg-purple-500 px-4 rounded-2xl">
            {Number(product.originalPrice || product.original_price || 0) > Number(product.price || 0) && (
              <div className="text-sm text-gray-200 line-through">฿{product.originalPrice ?? product.original_price}</div>
            )}
            <div className="text-xl font-bold text-white">{Number(product.price || 0) === 0 ? 'ฟรี' : `฿${product.price}`}</div>
          </div>

          <div className="text-right text-sm text-gray-300">
            <div>เหลือ: <span className="font-semibold text-white">{product.stock ?? product.qty ?? 0}</span> ชิ้น</div>
          </div>
        </div>

        <div className="flex gap-2">
          <button
            onClick={() => { if (outOfStock) return; router.push(`/products/${product.slug ?? product.id}`); }}
            disabled={outOfStock}
            aria-disabled={outOfStock}
            className={`flex-1 ${outOfStock ? 'bg-gray-700 text-gray-300 cursor-not-allowed' : 'bg-purple-600 hover:bg-purple-700 text-white'} font-medium py-2 px-4 rounded-lg transition-all duration-300 flex items-center justify-center gap-2`}
        ...
        <div className="text-xs text-center text-gray-300 mt-3">
          <div className="mt-1">จำหน่ายแล้ว: <span className="font-semibold text-white">{product.sold ?? 0}</span> ชิ้น</div>
        </div>
      </div>
    </div>
  );
